...
let MARKET_TIMEZONE_DELTA = +3; //What's the timezone offset to the market.
let OFFSET_TIME_24HRS = 24*60*60*1000;
...
(function(){
    ...
    if ("string" !== typeof TEST_DATE_STRING){
        now = new Date();
        let marketOpenHour = 10;
        marketOpenHour -= MARKET_TIMEZONE_DELTA;

        if (now.getHours() < marketOpenHour){
            // Too early to show quotes from today.  Show yesterday
            now = now.getTime();
            now -= OFFSET_TIME_24HRS; 
        }
    }
    else {
...
function updateStockBatch(status, index, count){
    ...
    try {
        ...
        if (Date.now() < (stockVO.lastUpdate + STOCK_UPDATE_INTERVAL)){
            ...
            let lastUpdateDate = new Date(stockVO.lastUpdate);
            let todayDate = new Date();
            if ( (lastUpdateDate.getDate() === todayDate.getDate())
                && (lastUpdateDate.getMonth() === todayDate.getMonth())
                && (lastUpdateDate.getFullYear() === todayDate.getFullYear()) ){
    ...
    if (!skip){
        getStockData(ticker, skipLastClose, handleUpdate); 
    }
    else {
    ...
    function handleUpdate(resp){
        try {
            ...
            console.error("Couldn't process stocks");
        }
    }
...
function getStockData(ticker, skipClose, cb){
    console.log("Fetching stock info: "+ticker);

    if (skipClose){
        console.log("Skipped closing price, already read recently");
        onDoneDaily({ skipped: true });
    }
    else {
        try {
...
function processStockData(ticker, svrresp){
    ...
    let closeData;
    if (!svrresp.daily.skipped){
        closeData = filterAndMapServerDailyResponse(svrresp.daily);
    }
    ...
    else if (Number.isNaN(stockData.prevClose)){
        console.error("Previous Day Close Data missing.  Need to re-read");
        stockData.lastUpdate = 0;
        stockData.prices = [stockData.prices[0]];
        stockData.prevClose = stockData.prices[0].open;
    }
